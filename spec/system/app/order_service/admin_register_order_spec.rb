...
describe "Admin register a new order_service" do
    it "with success" do
        # Arrange
        create_new_admin
        ...
        # got order_service code (because is generate automatically)
        @order_service_code = @shipping_company.order_services.last.code
    ...
    it "with empty fields" do
        ...
        fill_in "Sobrenome", with: "Lastname Destinatário"
        fill_in "RG", with: ""
        fill_in "Endereço completo (destinatário)", with: ""
        click_on "Cadastrar"

        # Assert
        expect(page).to have_content "Não foi possível cadastrar a ordem de serviço."
        expect(page).to have_content "Endereço completo (destinatário) não pode ficar em branco"
        expect(page).to have_content "RG não pode ficar em branco"
        expect(page).to have_content "RG deve possuir 9 caracteres"
    end

    it "with wrong RG field" do
        # Arrange
        create_new_admin
        @shipping_company = ShippingCompany.create!(brand_name: "ACME LTDA", corporate_name: "ACME", domain: "acme.com", registration_number: "xx.xxx.xxx/xxxx-xx", full_address: "123123", status: "Ativo")
        Carrier.create!(license_plate: "MKE-0945", brand: "Mercedes", code_model: "TRUCK-MERC-10", manufact_year: 2020, weight: 50000, shipping_company_id: 1)
